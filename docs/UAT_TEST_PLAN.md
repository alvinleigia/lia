...
| Appointment lifecycle | Live booking, reschedule, occupied-slot alternatives, cancellation, zero/one/multiple-match lookup, intervening-booking recovery and delayed confirmation passed. All three simultaneous-booking rounds passed after fixing recovery from a write-time slot conflict. | On `7465b76`, each race produced one booking and alternatives for the other caller, retained details on alternative selection and passed cleanup. Earlier open-chat/reload cases passed after 15 minutes 10 seconds. See `UAT_CALENDAR_2026-09-22.md` for timing, attempts and reports; the race sample does not certify arbitrary external writers or load levels. |
...
| 18 - Telnyx and extensions | Hosted milestones 18.9-18.13 complete; current scoped Project Chat lifecycle and simultaneous-booking UAT passed | Live WhatsApp and Telnyx are deferred, with Telnyx last. Preserve the separately deferred worker/backup-restore gates. |
...
2026-09-12 follow-up: the tester verified cancellation attempt #100 and calendar removal. Attempt #101 subsequently reported no matching appointment. A fresh booking message containing date, time, name, contact number and reason exposed an opening-message handoff bug: the selected task asked for the date again. The runtime now extracts the opening message under the selected published task. A stated clock time remains a preference until it matches exactly one fresh provider slot for that date and timezone; missing, unavailable or ambiguous times still require a choice. Confirmation remains required before booking. Automated coverage uses mocked model extraction and provider HTTP across Project Chat and the shared voice boundary; live one-message extraction remains a staging UAT check.

Active-task follow-up: a screenshot showed the full booking message entered after a date prompt, followed by a model-written confirmation question and a model failure on "yes". Two local regressions reproduced gaps in that path: candidate-bearing ordinary conversation bypassed required-operation confirmation, and requested-text binding collapsed multi-field candidates into one value. The fixes normalize candidate-bearing task answers and retain multi-field candidates. The requested-text shortcut also defers to extraction when the message contains explicit evidence for other missing typed fields; a focused engine test reproduced and verifies this path. A bare confirmation with complete validated fields but no saved review now prepares the real review without a model call or booking write; another confirmation is required to execute. The database test covers browser startup, an active text prompt, the full message, recovery without a saved review, and one confirmed provider write. Model and provider are mocked. The affected staging task/run diagnostics are still needed to establish which path produced the screenshot; live recovery remains to be tested.

Next manual test: start a fresh chat and supply the full 22 September booking request in one message. Expect a review containing the supplied details if 10:00 am Sydney is available, otherwise offered alternatives without repeating identity/reason questions. Confirm and verify the event. Repeat for 23 September to prepare the multiple-upcoming-appointment selection test. Switching from a failed cancellation lookup to booking inside the same active task is a separate unresolved UAT case. No task or action republish is needed for this runtime change.
